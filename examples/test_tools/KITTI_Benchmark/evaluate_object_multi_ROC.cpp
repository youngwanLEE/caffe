...
void saveAndPlotMulti(string dir_name,vector<string>& multi_dir_name,string file_name,string obj_type){
  //printf("check\n");
  char command[1024];
  //printf("check\n");
  string total_file_name = file_name+"_overall";
  //printf("store file: %s\n", total_file_name.c_str());

  // create png + eps
  for (int32_t j=0; j<2; j++) {

    // open file  
    FILE *fp = fopen((dir_name + "/" + total_file_name + ".gp").c_str(),"w");
    // dir_name = parent directory
    // file_name = "Car"
    // multi_dir_name = "VGG", "VGG_PED3", "VGG_ASP4", ...
    ...
    // plot error curve
    fprintf(fp,"plot ");
    for(int m = 0;m<multi_dir_name.size();m++){
      string each_file_name = dir_name + "/" + multi_dir_name[m] + "/" + file_name;
      fprintf(fp,"\"%s.txt\" using 1:2 title '\"%s\"' with lines ls %d lw %d,",each_file_name.c_str(), multi_dir_name[m].c_str(), m,lw);
    ...
    // run gnuplot => create png + eps
    sprintf(command,"cd %s; gnuplot %s",dir_name.c_str(),(total_file_name + ".gp").c_str());
    system(command);
  }

  // create pdf and crop
  sprintf(command,"cd %s; ps2pdf %s.eps %s_large.pdf",dir_name.c_str(),total_file_name.c_str(),total_file_name.c_str());
  system(command);
  sprintf(command,"cd %s; pdfcrop %s_large.pdf %s.pdf",dir_name.c_str(),total_file_name.c_str(),total_file_name.c_str());
  system(command);
  sprintf(command,"cd %s; rm %s_large.pdf",dir_name.c_str(),total_file_name.c_str());
  system(command);
}


bool eval(string result_sha, vector<string>& models){
  ...
  char* file_name_car="car_detection";
  char* file_name_ped="pedestrian_detection";
  char* file_name_cyc="cyclist_detection";

// and which labels where provided by this submission
  bool eval_car=true, eval_pedestrian=true, eval_cyclist=true;
  //printf("check\n");
  // eval cars
  if(eval_car){
//    saveAndPlotMulti(result_dir, models, CLASS_NAMES[CAR] + "_detection",CLASS_NAMES[CAR]);
        saveAndPlotMulti(result_dir, models, file_name_car,"car");
  }
  // eval pedestrians
  if(eval_pedestrian){
//    saveAndPlotMulti(result_dir, models, CLASS_NAMES[PEDESTRIAN] + "_detection", CLASS_NAMES[PEDESTRIAN]);
    saveAndPlotMulti(result_dir, models, file_name_ped, "pedestrian");
  }
  // eval cyclists
  if(eval_cyclist){
  //  saveAndPlotMulti(result_dir, models, CLASS_NAMES[CYCLIST] + "_detection",CLASS_NAMES[CYCLIST]);
    saveAndPlotMulti(result_dir, models, file_name_cyc, "cyclist");
  }
