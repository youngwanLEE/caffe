...
bool eval_class (FILE *fp_det, FILE *fp_ori, CLASSES current_class,const vector< vector<tGroundtruth> > &groundtruth,const vector< vector<tDetection> > &detections, bool compute_aos, vector<double> &precision, vector<double> &avgRecall, vector<double> &aos, DIFFICULTY difficulty) {
  ...
  for (int32_t i=0; i<N_TESTIMAGES/3; i++){
  ...
  avgRecall.assign(N_SAMPLE_PTS, 0); ///< for AR by LYW
  if(compute_aos)
  ...
  for (int32_t i=0; i<thresholds.size(); i++){
    ...
    avgRecall[i] = *max_element(avgRecall.begin()+i, avgRecall.end());
    if(compute_aos)
...
void saveAndPlotPlots(string dir_name,string file_name, string file_name2,string obj_type,vector<double> vals[],bool is_aos){
  ...
  for (int32_t ii = 0; ii < (int)N_SAMPLE_PTS; ii+=4){
      for (int s=0;s<3;s++){
        sum[s] += vals[s][ii];
      }
  }
  ...
  fclose(fp);
  for (int s=0;s<3;s++){
    average[s] = sum[s]/11.0;
  }
  FILE *fp_AP = fopen((dir_name + "/" + file_name2 + ".txt").c_str(),"w");
  fprintf(fp_AP, "Easy\t%f\nModerate\t%f\nHard\t%f\n", average[0], average[1], average[2]);
  fclose(fp_AP);
...
void saveAndPlotPlots2(string dir_name,string file_name, string file_name2,string file_name3,string obj_type,vector<double> vals[],vector<double> vals2[],bool is_aos){

  char command[1024];
  double sum[3] = {0};
  double sum_recall[3] = {0};
  double average[3] = {0};
  double average_recall[3] = {0};
  ...
  for (int32_t ii = 0; ii < (int)N_SAMPLE_PTS; ii++){
      for (int s=0;s<3;s++){
        sum_recall[s] += vals2[s][ii];
      }
  }

  for (int s=0;s<3;s++){
    average[s] = sum[s]/11.0;
    average_recall[s] = sum_recall[s]/(double)N_SAMPLE_PTS;
  }
  FILE *fp_AP = fopen((dir_name + "/" + file_name2 + ".txt").c_str(),"w");
  fprintf(fp_AP, "Easy\t%f\nModerate\t%f\nHard\t%f\n", average[0], average[1], average[2]);
  fclose(fp_AP);

  FILE *fp_AR = fopen((dir_name + "/" + file_name3 + ".txt").c_str(),"w");
  fprintf(fp_AR, "Easy\t%f\nModerate\t%f\nHard\t%f\n", average_recall[0], average_recall[1], average_recall[2]);
  fclose(fp_AR);
...
bool eval(string result_sha,Mail* mail){
  ...
  // excution path : /home/cvlab/Workspace/03_Evaluation/Original/cpp/
  string gt_dir         = "/home/youngwan/caffe/data/kitti_ssd/test_annotation";
  ...
  for (int32_t i=0, idx = 0; i<N_TESTIMAGES, idx<N_TESTIMAGES; i++, idx+=3) {

    // file name
    char file_name[256];
    sprintf(file_name,"%06d.txt",idx);
  ...
  if(eval_car){
    ...
    //   return false;
    // }
    vector<double> precision[3],recall[3], aos[3];
    if(   !eval_class(fp_det,fp_ori,CAR,groundtruth,detections,compute_aos,precision[0],recall[0],aos[0],EASY)
       || !eval_class(fp_det,fp_ori,CAR,groundtruth,detections,compute_aos,precision[1],recall[1],aos[1],MODERATE)
       || !eval_class(fp_det,fp_ori,CAR,groundtruth,detections,compute_aos,precision[2],recall[2],aos[2],HARD)){
      mail->msg("Car evaluation failed.");
      return false;
    }
    fclose(fp_det);
    // saveAndPlotPlots(plot_dir,CLASS_NAMES[CAR] + "_detection", CLASS_NAMES[CAR] + "_det_AP",CLASS_NAMES[CAR],precision,0);
    saveAndPlotPlots2(plot_dir,CLASS_NAMES[CAR] + "_detection", CLASS_NAMES[CAR] + "_det_AP",CLASS_NAMES[CAR] + "_det_AR",CLASS_NAMES[CAR],precision,recall,0);
    if(compute_aos){
      saveAndPlotPlots(plot_dir,CLASS_NAMES[CAR] + "_orientation",CLASS_NAMES[CAR] + "_aos_AP",CLASS_NAMES[CAR],aos,1);
      fclose(fp_ori);
    }
  }
  ...
  if(eval_pedestrian){
    ...
    // }
    vector<double> precision[3],recall[3], aos[3];
    if(   !eval_class(fp_det,fp_ori,PEDESTRIAN,groundtruth,detections,compute_aos,precision[0],recall[0],aos[0],EASY)
       || !eval_class(fp_det,fp_ori,PEDESTRIAN,groundtruth,detections,compute_aos,precision[1],recall[1],aos[1],MODERATE)
       || !eval_class(fp_det,fp_ori,PEDESTRIAN,groundtruth,detections,compute_aos,precision[2],recall[2],aos[2],HARD)){
      mail->msg("Pedestrian evaluation failed.");
      return false;
    }
    fclose(fp_det);
    //saveAndPlotPlots(plot_dir,CLASS_NAMES[PEDESTRIAN] + "_detection", CLASS_NAMES[PEDESTRIAN] + "_det_AP",CLASS_NAMES[PEDESTRIAN],precision,0);
    saveAndPlotPlots2(plot_dir,CLASS_NAMES[PEDESTRIAN] + "_detection", CLASS_NAMES[PEDESTRIAN] + "_det_AP",CLASS_NAMES[PEDESTRIAN] + "_det_AR",CLASS_NAMES[PEDESTRIAN],precision,recall,0);
    if(compute_aos){
      fclose(fp_ori);
      saveAndPlotPlots(plot_dir,CLASS_NAMES[PEDESTRIAN] + "_orientation",CLASS_NAMES[PEDESTRIAN] + "_aos_AP",CLASS_NAMES[PEDESTRIAN],aos,1);
    }
  }
  ...
  if(eval_cyclist){
    ...
    // }
    vector<double> precision[3],recall[3], aos[3];
    if(   !eval_class(fp_det,fp_ori,CYCLIST,groundtruth,detections,compute_aos,precision[0],recall[0],aos[0],EASY)
       || !eval_class(fp_det,fp_ori,CYCLIST,groundtruth,detections,compute_aos,precision[1],recall[1],aos[1],MODERATE)
       || !eval_class(fp_det,fp_ori,CYCLIST,groundtruth,detections,compute_aos,precision[2],recall[2],aos[2],HARD)){
      mail->msg("CYCLIST evaluation failed.");
      return false;
    }  
    fclose(fp_det);
    //saveAndPlotPlots(plot_dir,CLASS_NAMES[CYCLIST] + "_detection", CLASS_NAMES[CYCLIST] + "_det_AP",CLASS_NAMES[CYCLIST],precision,0);
    saveAndPlotPlots2(plot_dir,CLASS_NAMES[CYCLIST] + "_detection", CLASS_NAMES[CYCLIST] + "_det_AP",CLASS_NAMES[CYCLIST] + "_det_AR",CLASS_NAMES[CYCLIST],precision,recall,0);
    if(compute_aos){
      fclose(fp_ori);
      saveAndPlotPlots(plot_dir,CLASS_NAMES[CYCLIST] + "_orientation",CLASS_NAMES[CYCLIST] + "_aos_AP",CLASS_NAMES[CYCLIST],aos,1);
    }
  }
